...
Note that you should not put this function in your R Markdown document, but instead type it directly in the Console, as it pops open a new window (and the concept of popvping open a window in a static document doesn't really make sense...).
...
Each row represents an Airbnb listing in Edinburgh.
...
I think that it makes more sense to wrap the facets because it gives an organized, spaced out view and it makes room for all of the neighborhood names for each histogram.  I previously tried to lay the facets out using the facet_grid function and it looked less organized, plus there was not much room for the names of the towns.  This visualization is much easier to read and understand and it looks better.
...
```{r eval=TRUE}
library(ggridges)
ggplot(edibnb, aes(x = price, y = neighbourhood, fill = neighbourhood)) +
...
Airbnb guests in Edinburgh typically rate properties in the 75-100 range.  In places like Leith, Old Town, Marchmont, and Haymarket, most guests rated properties 100.  Meanwhile in Old Town, most guests
rated the properties in the 80-90 range.
